...
int	engine::luaL_loadstring						(lua_State* L, char const* s)
{
	return	::luaL_loadstring(L, s);
}

int	engine::luaL_newmetatable					(lua_State* L, char const* tname)
{
...
int	engine::lua_getinfo							(lua_State* L, char const* what, lua_Debug* ar)
{
...
void engine::lua_getfield						(lua_State* L, int idx, char const* k)
{
	return	::lua_getfield(L, idx, k);
}

char const*	engine::lua_getlocal				(lua_State* L, lua_Debug const* ar, int n)
{
...
void engine::lua_pushstring						(lua_State* L, char const* s)
{
...
void const* engine::lua_topointer				(lua_State* L, int idx)
{
...
{
	ASSERT	(m_instance_count < sizeof(m_instances)/sizeof(m_instances[0]));
...
{
	CS_UNREFERENCED_PARAMETER	( message_type );
	CS_UNREFERENCED_PARAMETER	( message );
}
...
void engine::type_convert_class						(char* const buffer, unsigned int const size, lua_State* state, int index)
{
	...
	ASSERT					(object, "invalid object userdata");
...
{
	CS_UNREFERENCED_PARAMETER	( use_in_description );
...
void engine::fill_class_info	(cs::lua_studio::backend& backend, char* const buffer, unsigned int const size, luabind::detail::object_rep* object, luabind::detail::class_rep* class_rep, lua_State* state)
{
	...
	stream					+= cs::sprintf(stream, size - u32(stream - buffer), "{");
	...
	for (u32 i=0; I != E; ++I, ++i) {
		...
		if (use_in_description)
			stream				+= cs::sprintf(stream, size - u32(stream - buffer), "%s[%s]=%s ", (*I).first, type, value);
		else
			stream				+= cs::sprintf(stream, size - u32(stream - buffer), "%s=%s ", (*I).first, value);
	}

	stream						+= cs::sprintf(stream, size - u32(stream - buffer), "}%c",0);
}
...
bool engine::value_convert_instance	(cs::lua_studio::backend& backend, char* buffer, unsigned int size, luabind::detail::object_rep* object, lua_State* state)
{
	...
	for (i=0; lua_next(state,-2); ++i) {
		...
		if (use_in_description)
			stream			+= cs::sprintf(stream, size - u32(stream - buffer), "%s[%s]=%s ", name, type, value);
		else
			stream			+= cs::sprintf(stream, size - u32(stream - buffer), "%s=%s ", name, value);
		...
		case engine::lua_type_user_data	: {
			if (!luabind::detail::is_class_object( state, index )) {
				if (!is_luabind_class(state, index)) {
					...
					cs::sprintf		(buffer, size, "0x%08x", *(u32 const*)&user_data);
					return			true;
				}
...
void engine::push_class							(lua_State* const state, char const* const id)
{
	...
	R_ASSERT					(class_rep, "null class userdata");
...
void engine::push_class_base					(lua_State* const state, char const* const id)
{
	...
	typedef luabind::vector<base_info>		Bases;
	...
	)
{
	...
	R_ASSERT						(_class, "invalid class userdata");

	{
		string4096					type;
		typedef luabind::detail::class_rep::base_info	base_info;
		luabind::vector<base_info>::const_iterator	i = _class->bases().begin();
		luabind::vector<base_info>::const_iterator	e = _class->bases().end();
		for ( ; i != e; ++i)
	...
	)
{
	...
	R_ASSERT					(class_object, "invalid class userdata");
...
struct value_info {
	...
	pcstr						m_type;
	pcstr						m_value;
	cs::lua_studio::icon_type	m_icon_type;
}; // struct value_info

struct value {
	static inline bool predicate ( value_info const& left, value_info const& right )
	{
		return				sz_cmp( left.m_name, right.m_name ) < 0 ;
	}
}; // struct value
	...
	)
{
	...
	u32 count					= 0;
	lua_pushnil					(state);
	for ( ; lua_next(state,-2); ++count )
		lua_pop_value			(state, 1);

	string4096					temp;
	cs::detail::value_info* const values	= (cs::detail::value_info*)ALLOCA( count*sizeof( cs::detail::value_info ) );
	lua_pushnil					(state);
	for (cs::detail::value_info* i = values ; lua_next(state,-2); ++i ) {
		CS_NEW_DETAIL_COPY_CONSTRUCTOR0::initialize	( i );
		( *i ).m_name			= lua_to_string(state,-2);

		bool					use_in_description;
		backend.type_to_string	(temp, sizeof(temp), state, -1, use_in_description);
		( *i ).m_type			= temp;

		backend.value_to_string	(temp, sizeof(temp), state, -1, ( *i ).m_icon_type, true);
		( *i ).m_value			= temp;
	...
	std::sort					( values, values + count, cs::detail::value::predicate );

	for ( cs::detail::value_info* i = values, *e = values + count; i != e; ++i ) {
		value_to_expand.add_value	(
			( *i ).m_name,
			( *i ).m_type,
			( *i ).m_value,
