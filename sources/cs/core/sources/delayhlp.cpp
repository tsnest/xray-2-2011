...
static inline int
__memcmp(void const*  pv1, void const*  pv2, size_t cb) {
    if (!cb) {
        return 0;
        }
...
static inline void* 
__memcpy(void*  pvDst, void const*  pvSrc, size_t cb) {
...
const IMAGE_DOS_H_INCLUDEDEADER __ImageBase;
#else
extern "C"
const IMAGE_DOS_H_INCLUDEDEADER __ImageBase;
#endif
...
typedef InternalImgDelayDescr const*    PCIIDD;

static inline
PIMAGE_NT_H_INCLUDEDEADERS WINAPI
PinhFromImageBase(HMODULE hmod) {
    return PIMAGE_NT_H_INCLUDEDEADERS(PBYTE(hmod) + PIMAGE_DOS_H_INCLUDEDEADER(hmod)->e_lfanew);
    }
...
static inline
DWORD WINAPI
TimeStampOfImage(PIMAGE_NT_H_INCLUDEDEADERS pinh) {
    return pinh->FileHeader.TimeDateStamp;
    }

static inline
bool WINAPI
FLoadedAtPreferredAddress(PIMAGE_NT_H_INCLUDEDEADERS pinh, HMODULE hmod) {
...
#pragma warning(suppress:6014)
    ...
    if (pfnRet == 0) {
        if (pidd->rvaBoundIAT && pidd->dwTimeStamp) {
            ...
            //
            PIMAGE_NT_H_INCLUDEDEADERS   pinh(PinhFromImageBase(hmod));
...
extern "C"
HRESULT WINAPI
__H_INCLUDEDrLoadAllImportsForDll(LPCSTR szDll) {
    HRESULT             hrRet = HRESULT_FROM_WIN32(ERROR_MOD_NOT_FOUND);
    PIMAGE_NT_H_INCLUDEDEADERS   pinh = PinhFromImageBase(HMODULE(&__ImageBase));
